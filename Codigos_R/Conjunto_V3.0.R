roda_bacia <-function(bacia,dia_previsao,tempo_regressao,dias_previstos,agrupamento,modelos,alpha,beta,lambdas){
    ...
    precs<-lapply(modelos,function(x){
      ...
      for( i in 201:1){
        if (file.exists(paste0("./Arq_Entrada/",x,"/",x,"_m_",format(dia_previsao-i+1,"%d%m%y"),".dat"))){
          ...
          pr<-rbind(pr,dia) #pr = vetor [dia da previsao, valor prev prev]
        } else { paste0(x,"_m_",format(dia_previsao-i+1,"%d%m%y"),".dat n�o existe")}
      }
      pr
    })
  ...
  #===================================== checa se a soma dos agrupamentos � igual ao n�mero de dias previstos=====================
  if (sum(agrupamento)!=dias_previstos){stop("ERRO DE PARAMETRO: soma dos dias dos agrupamentos diferente do n�mero de dias previstos.")}
  #=====================================checa se para cada agrupamento o n�mero de modelos � igual================================
  for(j in 1:length(agrupamento)){
    for( i in (1+sum(agrupamento[1:j])-agrupamento[j]):(sum(agrupamento[1:j]))){
      if (num_mod_hor[i]!= num_mod_hor[1+sum(agrupamento[1:j])-agrupamento[j]]){stop(paste0("ERRO: N�mero diferente de modelos no ",j,"� agrupamento para a bacia ",bacia))}
    }
  }
  ...
  for( i in 200:2){
    ...
    if (file.exists(paste0("./Arq_Entrada/Observado/psat_",format(dia_previsao-i+1,"%d%m%Y"),".txt"))){
      leitura<-read.table(paste0("./Arq_Entrada/Observado/psat_",format(dia_previsao-i+1,"%d%m%Y"),".txt"),header=F,stringsAsFactors	=F)  
  ...
  #======================checa se existe previs�o para o dia da previs�o=================================
  for (i in 1:(length(modelos))){if (length(which(precs[[i]][,1]==dia_previsao))==0){stop (paste0("o modelo " , modelos[i], " n�o possui a data de de previs�o para a bacia:",bacia))}}
  ...
  for (i in 1:dias_previstos){
    ...
    while(soma<=tempo_regressao){
      dia<-dia_previsao -k # dia sendo buscado para ver se a regress�o existe
      ...
      if (maior_data > dia) {
        ...
        stop("Número insuficiente de elementos para o tamanho da regressão solicitada")
      }
      k=k+1
    }
  }
...
#==========================================================bloco que depois vai virar a fun��o======================================================
library(lpSolve)
  ...
  if(sp==0) {fator<-1}else{
    ...
    if (modelo[1]>0){
      if (x_95>sp){ # bloco para quando a soma estiver abaixo de 95% da amostra
        ...
        if (x_95>(y_95)){ # bloco para quando soma acima de 95%, por�m abaixo da reta x=y
    ...
    if (modelo[1]<0){
      if (x_95>(y_95)){ # bloco para quando y95 est� abaixo da reta x=y
        if (x_max>sp){
          fator<-(sp*modelo[2]+modelo[1]*sp^2)/sp
        }else{ fator<- y_max/sp}
      } else { # bloco para quando y95 est� acima da reta x=y ou sobre a reta 
        if (sp<x_max){
          if (sp< ((sp*modelo[2]+modelo[1]*sp^2))){
            fator<-(sp*modelo[2]+modelo[1]*sp^2)/sp
          }else{fator<-1} 
